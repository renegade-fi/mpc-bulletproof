//! Definition of the constraint system trait within an MPC network.

use crate::r1cs::{LinearCombination, R1CSError, Variable};
use curve25519_dalek::scalar::Scalar;
use merlin::Transcript;
use mpc_ristretto::{beaver::SharedValueSource, mpc_scalar::MpcScalar, network::MpcNetwork};
...
///
/// Within the MPC network, this trait has two bounds. `N` represents a
/// networking layer that the group elements are shared over. `S` represents
/// the infrastructure used to implement a Beaver style pre-processing
/// functionality.
pub trait ConstraintSystem<N: MpcNetwork + Send, S: SharedValueSource<Scalar>> {
    ...
    fn allocate(&mut self, assignment: Option<MpcScalar<N, S>>) -> Result<Variable, R1CSError>;
    ...
    fn allocate_multiplier(
        &mut self,
        input_assignments: Option<(MpcScalar<N, S>, MpcScalar<N, S>)>,
...
pub trait RandomizableConstraintSystem<N, S>: ConstraintSystem<N, S>
where
    N: MpcNetwork + Send,
    S: SharedValueSource<Scalar>,
{
    /// Represents a concrete type for the CS in a randomization phase.
    type RandomizedCS: RandomizedConstraintSystem<N, S>;
...
pub trait RandomizedConstraintSystem<N, S>: ConstraintSystem<N, S>
where
    N: MpcNetwork + Send,
    S: SharedValueSource<Scalar>,
{
    ...
    /// })
    /// ```
    fn challenge_scalar(&mut self, label: &'static [u8]) -> MpcScalar<N, S>;
}
